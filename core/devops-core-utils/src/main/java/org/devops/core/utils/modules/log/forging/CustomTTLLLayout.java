 ...
 * @since 1.1.6
 */
public class CustomTTLLLayout extends LayoutBase<ILoggingEvent> {
    ...
    public String doLayout(ILoggingEvent event) {
        ...
        sb.append(" - ");
        sb.append(covertLog(event.getFormattedMessage()));
    ...
    private String covertLog(String message){
        if (StringUtil.isEmpty(message)) {
            return message;
        }
        String normalize = Normalizer.normalize(message, Normalizer.Form.NFKC);
        for (Map.Entry<String, String> entry : BLACKLIST.entrySet()) {
            normalize = normalize.replaceAll(entry.getKey(), entry.getValue());
        }
        return normalize;
    }
