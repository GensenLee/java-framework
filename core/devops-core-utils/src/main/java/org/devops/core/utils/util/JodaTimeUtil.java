...
public class JodaTimeUtil {
     ...
     * @return
     */
    private static String date2String(DateTime dateTime, String format) {
    ...
    private static String than2Today(String format, Integer days, Integer hours, Integer minutes, Integer seconds) {
        ...
        //查找天数时间
        if (days != null) {
            dayThanToday = dateTime.minusDays(-days);
        }
        //查找小时时间
        if (hours != null) {
            dayThanToday = dateTime.minusHours(-hours);
        }
        //查找分钟时间
        if (minutes != null) {
            dayThanToday = dateTime.minusMinutes(-minutes);
        }
        //查找秒时间
        if (seconds != null) {
            dayThanToday = dateTime.minusSeconds(-seconds);
        }

        if (dayThanToday == null) {
            return "";
        }
