...
public class SwaggerUiController {
	...
	private SwaggerBootstrapUi initSwaggerBootstrapUi(HttpServletRequest request, Documentation documentation) {
		...
		while (tags.hasNext()) {
			...
			for (Map.Entry<String, Object> entry : beansWithAnnotation.entrySet()) {
				...
				api = AnnotationUtils.findAnnotation(aClass,Api.class);
				if (api != null) {
					// 是否相等
			...
			if (exists) {
				...
				if (post == 0) {
					ApiSort annotation = AnnotationUtils.findAnnotation(aClass,ApiSort.class);
					if (annotation != null) {
				...
				// 获取父级path
				String parentPath = "";
				RequestMapping parent = AnnotationUtils.findAnnotation(aClass,RequestMapping.class);
				if (parent != null) {
	...
	private Swagger extend(Swagger swagger) {
		...
		while (it.hasNext()) {
			...
			if (key.contains("ApiResult")) {
				Map<String, Property> props = model.getProperties();
				Property dataProp = props.get("object");
				Property newProp = SwaggerUtil.getNewProp(dataProp, SwaggerUtil.getRealType(key), swagger.getDefinitions());
				props.put("object", newProp);
			}else if (key.contains("PageResult")) {
				Map<String, Property> props = model.getProperties();
				Property dataProp = props.get("list");
				Property newProp = SwaggerUtil.getNewProp(dataProp,"list", SwaggerUtil.getRealType(key), swagger.getDefinitions());
				props.put("list", newProp);
			}
		}
		return swagger;
	}
