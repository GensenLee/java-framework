...
public class SwaggerUtil {
	...
	public static Property getNewProp(Property dataProp, String type, Map<String, Model> definitions) {
		return getNewProp(dataProp,type,"",definitions);
	}
	/**
	 * 递归处理泛型类型 <br>
	 ...
	 * @return
	 */
	public static Property getNewProp(Property dataProp, String type, String rType,Map<String, Model> definitions) {
		...
		Model model = definitions.get(rType);
		...
		if (SwaggerUtil.isMap(type)) {
			...
			((RefProperty) newProp).set$ref(rType);
		} else if (SwaggerUtil.isIterable(type)) {
			String ref = SwaggerUtil.getRealType(rType);
			newProp = new ArrayRefProperty();
			BeanUtils.copyProperties(dataProp, newProp);
			((ArrayRefProperty) newProp).set$ref(ref);
			((ArrayRefProperty) newProp).setType(ArrayProperty.TYPE);
			if (!realType.equals(type)) {
