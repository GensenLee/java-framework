package org.devops.core.utils.util.identifier;
...
public final class TSID {
    ...
    public static String next() {
        ...
        if (ct <= threadLocal.lastTimestamp) {
            ...
            long n = (((long) nh) << 32 | (nl & 0xFFFFFFFFL)) + (1 << 6);
        ...
        ids[11] = ENCODING_CHARS[((int) (ct) & 0x07) << 2 | ((entropy[5] >> 6) & 0x03)]; // 3 + 2 -> 50
        ids[12] = ENCODING_CHARS[(entropy[5] >> 1) & 0x1F]; // 5 + 0
        ids[13] = ENCODING_CHARS[((entropy[5] & 0x01) << 4) | ((entropy[4] >> 4) & 0x0F)]; // 1 + 4
        ids[14] = ENCODING_CHARS[((entropy[4] & 0x0F) << 1) | ((entropy[3] >> 7) & 0x01)]; // 4 + 1
        ids[15] = ENCODING_CHARS[(entropy[3] >> 2) & 0x1F]; // 5 + 0
        ids[16] = ENCODING_CHARS[((entropy[3] & 0x03) << 3) | (entropy[2] >> 5) & 0x07]; // 2 + 3
        ids[17] = ENCODING_CHARS[entropy[2] & 0x1F]; // 5 + 0
        ids[18] = ENCODING_CHARS[(entropy[1] >> 3) & 0x1F]; // 5 + 0
        ids[19] = ENCODING_CHARS[((entropy[1] & 0x07) << 2) | ((entropy[0] >> 6) & 0x03)]; // 3 + 2
