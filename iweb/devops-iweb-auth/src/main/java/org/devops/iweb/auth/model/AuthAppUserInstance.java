...
public class AuthAppUserInstance extends BaseBean{
	...
	public static List<AuthAppUserInstance> init(){
		...
		List<AuthAppUserInstance> list = new ArrayList<>();
		list.add(new AuthAppUserInstance(1L,1L,-1L,1L,1,0,0,"system",now,"system",now));
		return list;
	}
	
	public static String initSQL() {
		return ResourceUtil.readResourceAsString("classpath:sql/devops-iweb-auth/auth_app_user_instance.sql");
	}
