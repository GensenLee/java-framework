...
public class AuthAppInstance extends BaseBean{
	...
	/** 父实例id */
	public static final String PARENT_APP_INSTANCE_ID = "parent_app_instance_id";
	...
	@Column(name = "parent_app_instance_id",jdbcType="bigint(20)",notNull=true,def="-1",comment="父实例流水号")
	@VerifyField(ignore = true)
	private Long parentAppInstanceId;

	public static List<AuthAppInstance> init(){
		final Date now = new Date();
		List<AuthAppInstance> list = new ArrayList<>();
		list.add(new AuthAppInstance(1L,-1L,"运营支撑系统",0,"system",now,"system",now,-1L));
		return list;
	}
	
	public static String initSQL() {
		return ResourceUtil.readResourceAsString("classpath:sql/devops-iweb-auth/auth_app_instance.sql");
	}
	
	public AuthAppInstance(Long appId, Long companyId, String name, Integer delFlag, String createName,
			Date createTime, String modifyName, Date modifyTime,Long parentAppInstanceId) {
		super();
		this.appId = appId;
		...
		this.parentAppInstanceId = parentAppInstanceId;
	}
