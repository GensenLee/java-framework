...
public class AuthApp extends BaseBean{
	...
	public static List<AuthApp> init(){
		...
		List<AuthApp> list = new ArrayList<>();
		list.add(new AuthApp("inner_system","内部系统","内部系统-包括内部SUC子系统","javascript:void(0)",0,1,"system",now,"system",now));
		return list;
	}
	
	public static String initSQL() {
		return ResourceUtil.readResourceAsString("classpath:sql/devops-iweb-auth/auth_app.sql");
	}
